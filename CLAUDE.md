...
- **NEVER externalize/bundle `react-router-dom`.** It must be consumed via `context.ReactRouter` (Local provides it). An `external` turns `import … from 'react-router-dom'` into an eager top-of-bundle `require("react-router-dom")`; on a clean install Local loads the renderer with a plain Node `require()` from the add-on dir (outside its asar), where react-router-dom does not resolve → `MODULE_NOT_FOUND` at module-eval → the add-on's `export default` never runs → **no hooks register (no UI), surfacing as Local's render-time `Cannot read properties of undefined (reading 'toString')` crash**. It only "worked" on the dev machine because the installed add-on is a symlink to the repo (full `node_modules`). Guarded by `src/renderer/bundle.packaging.test.ts`, which asserts every non-relative `require()` in the built `lib/renderer/index.js` is runtime-resolvable on a clean install (Local-provided / electron / `bundledDependencies` only).
