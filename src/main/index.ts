...
async function waitForKinstaOperation(
...
async function createKinstaBackup(
  client: AxiosInstance,
  envId: string,
  sync: ActiveSync,
      ...
      try {
        ...
        if (options.includeDatabase) {
          ...
          await runCommand(
            sync,
            'ssh',
            sshArgs(envInfo, `cd ~/public && wp db export ${remoteDbPath}`),
          );
      ...
      try {
        ...
        if (options.deletions && options.deletions.length > 0) {
          ...
          for (let i = 0; i < quoted.length; i += 200) {
            ...
            await runCommand(
              sync,
              'ssh',
              sshArgs(envInfo, `cd ~/public && rm -rf -- ${batch.join(' ')}`),
            );
          }
        }
        ...
        if (options.includeDatabase) {
          ...
          await runCommand(
            sync,
            'ssh',
            sshArgs(envInfo, `cd ~/public && wp db import ${remoteDbPath}`),
          );
        ...
        if (remoteImportStarted) {
          try {
            ...
            await runCommand(
              { cancelled: false, child: null },
              'ssh',
              sshArgs(envInfo, 'cd ~/public && wp db import ~/kinsta-sync-pre-push-backup.sql'),
            );
            notify(
              'Kinsta Sync',
            ...
            notify(
              'Kinsta Sync',
              'Push aborted — automatic remote DB restore FAILED. Backup on server: ~/kinsta-sync-pre-push-backup.sql',
            );
          }
        }
        if (error instanceof CancelledError) {
...
async function clearKinstaCaches(
  client: AxiosInstance,
  envId: string,
  cdnCacheId?: string,
): Promise<string[]> {
